...
public class TvrtkaSingleton {
	...
	public void ispisiVlak(String oznaka) {
		...
		for (int i = 0; i < komponente.size(); i++) {
			...
			System.out.printf("%-12s %-13s %-22s %-22s %-16s %-16s %-10s %-11s %n", vlak.oznaka, etapa.pruga.oznakaPruge, etapa.pocetnaStanica.stanica, etapa.zavrsnaStanica.stanica, etapa.vratiPocetnoVrijeme(), etapa.vratiZavrsnoVrijeme(), etapa.vratiKm(), etapa.raspored.vratiDane());
